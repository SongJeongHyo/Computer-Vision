...
void zhangSuen(unsigned char* image, unsigned char* output, int rows, int cols) {
	...
	for (i = 0; i < rows; i++) {
		...
		for (int k = 0; k < cols; k++) 
			imageMatrix[i][k] = image[i * cols + k];
	}
...
void FeatureExtractThinImage(BYTE* Image, BYTE* Output, int W, int H, int row, int col) {
	...
	for (int i = 1; i < H - 1; i++) {
		for (int j = 1; j < W - 1; j++) {
			if (i != 0 || j != 0) {
				...
				sum = ((Image[(i - 1) * W + j] == blankPixel && Image[(i - 1) * W + (j + 1)] == imagePixel)
					+ (Image[(i - 1) * W + (j + 1)] == blankPixel && Image[i * W + (j + 1)] == imagePixel)
					+ (Image[i * W + (j + 1)] == blankPixel && Image[(i + 1) * W + (j + 1)] == imagePixel)
					+ (Image[(i + 1) * W + (j + 1)] == blankPixel && Image[(i + 1) * W + j] == imagePixel)
					+ (Image[(i + 1) * W + j] == blankPixel && Image[(i + 1) * W + (j - 1)] == imagePixel)
					+ (Image[(i + 1) * W + (j - 1)] == blankPixel && Image[i * W + (j - 1)] == imagePixel)
					+ (Image[i * W + (j - 1)] == blankPixel && Image[(i - 1) * W + (j - 1)] == imagePixel)
					+ (Image[(i - 1) * W + (j - 1)] == blankPixel && Image[(i - 1) * W + j] == imagePixel));

				//sum = getBWTransitions(row, col);
...
int main()
{
	...
	FeatureExtractThinImage(Temp, Image, W, H, 3, 3);
